-- Migration: order_processing_enhancements_fixed
-- Created at: 1755433072

-- Corrected Incremental Order Processing Workflow Enhancements
...
CREATE INDEX IF NOT EXISTS idx_orders_priority ON orders(order_priority);
...
CREATE INDEX IF NOT EXISTS idx_orders_assigned_processor ON orders(assigned_processor_id);
...
-- Create views for order management dashboard (corrected to use existing table structure)
...
LEFT JOIN order_exceptions oe ON o.id = oe.order_id AND oe.status IN ('open', 'in_progress')
