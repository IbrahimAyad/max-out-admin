-- Migration: create_enhanced_schema_clean
-- Created at: 1755577066

-- Drop existing tables if they exist
DROP TABLE IF EXISTS low_stock_alerts CASCADE;
DROP TABLE IF EXISTS inventory_history CASCADE;
DROP TABLE IF EXISTS enhanced_product_variants CASCADE;
DROP TABLE IF EXISTS sizing_categories CASCADE;
...
CREATE INDEX idx_enhanced_variants_product_id ON enhanced_product_variants(product_id);
CREATE INDEX idx_enhanced_variants_sku ON enhanced_product_variants(sku);
CREATE INDEX idx_enhanced_variants_stock_status ON enhanced_product_variants(stock_status);
CREATE INDEX idx_enhanced_variants_color_size ON enhanced_product_variants(color, size);
