-- Migration: create_blazer_variants_fixed
-- Created at: 1755743566
...
blazer_variants AS (
  SELECT 
    ...
    'BLZ-' || UPPER(SUBSTRING(REGEXP_REPLACE(p.name, '[^a-zA-Z0-9]', '', 'g'), 1, 8)) || '-' || UPPER(v.color) || '-' || v.size as sku
...
SELECT 
  ...
  CASE 
    WHEN color IN ('Navy', 'Black') THEN 12  -- Higher stock for popular colors
    ELSE 6  -- Standard stock
  ...
  1800 as weight_grams  -- ~1.8kg for blazer
